...
- 書き込み先セルが**既に数式(formula)を持っている場合は書き込みを拒否する**。
  ...
  これを知らずに上書きすると数式が失われる(実際に本スクリプトのテストで発生した事故)。
...
例(テーブル一覧.xlsxのシート「1」へ、マッピングJSON mapping.jsonの内容を反映する場合):
    python apply_mapping.py テーブル一覧.xlsx mapping.json 出力.xlsx
"""
...
def apply_mapping(
    ...
    mapping,
    output_path,
    allow_outside_border=False,
    force_formula_overwrite=False,
):
    ...
    planned = (
        []
    )  # (sheet_name, anchor, value) のうち検証を通過したもの。この段階ではまだ書き込まない。
    ...
    for sheet_name, cell_map in mapping.get("sheets", {}).items():
        ...
        # 同じ呼び出し内で書き換える場合でも、更新後の選択肢と照合できる。
...
def main():
    ...
    report = apply_mapping(
        ...
        mapping,
        args.output_path,
        args.allow_outside_border,
        args.force_formula_overwrite,
    )

    if report["errors"]:
        print(
            f"=== エラー {len(report['errors'])}件 → 1セルも書き込まずに中止しました ==="
        )
    ...
    if report["saved"]:
        ...
        print(
            "=== 書き込み結果: 保存なし（マッピングJSONに有効な書き込みが1件もありませんでした） ==="
        )
    print(f"書き込みセル数: {len(report['written'])}")
    for w in report["written"]:
        mark = (
            " (結合セルのため書き込み先を変更)"
            if w["cell"] != w["requested_cell"]
            else ""
        )
        print(
            f"  {w['sheet']}!{w['cell']}{mark} : {w['old_value']!r} -> {w['new_value']!r}"
        )
